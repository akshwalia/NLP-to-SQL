        ...
        <div 
          className={`px-5 py-4 rounded-2xl shadow-sm transition-all duration-300 group-hover:shadow-lg transform group-hover:-translate-y-0.5 ${
            isUser
              ...
              : isConversational
                ? 'bg-gray-800 border border-gray-700 text-gray-100 hover:bg-gray-750 hover:border-gray-600'
                : 'bg-gray-800 border border-gray-700 text-gray-100 hover:bg-gray-750 hover:border-gray-600'
          }`}
        >
          <div 
            className={`prose prose-sm max-w-none ${
              isUser 
                ...
                : 'prose-invert [&_*]:text-gray-100'
            } prose-table:table-auto prose-table:w-full prose-td:p-2 prose-th:p-2 prose-thead:bg-gray-700 prose-tr:border-b prose-tr:border-gray-600`}
          >
            <ReactMarkdown remarkPlugins={[remarkGfm]}>{processedContent}</ReactMarkdown>
          </div>
        </div>
        <div 
          className={`text-xs text-gray-400 mt-2 transition-all duration-300 group-hover:text-gray-300 ${
